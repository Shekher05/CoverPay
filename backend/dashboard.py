...
from backend.database import IncidentRecord, Prediction, Transaction, get_session
...
def incidents(
    ...
    rows = (
        session.execute(
            select(IncidentRecord).order_by(IncidentRecord.opened_dt.desc()).limit(limit)
        )
        .scalars()
        .all()
    )
    ...
    return sorted(
        ...
        key=lambda i: (SEVERITY_RANK.get(i["severity"], 9), -i["opened_dt"]),
    )
