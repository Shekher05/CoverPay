...
def generate_batch(
    ...
    picks = pd.concat(
        [
            legit_pool.sample(
                rows - n_fraud, replace=True, random_state=int(rng.integers(1_000_000_000))
            ),
            fraud_pool.sample(
                n_fraud, replace=True, random_state=int(rng.integers(1_000_000_000))
            ),
        ]
    )

    out = picks.sample(frac=1, random_state=int(rng.integers(1_000_000_000))).reset_index(drop=True)
