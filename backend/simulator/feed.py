...
from ml.inference import get_model, reasons_for_row
...
def _reasons(contribs, features, position: int, columns: list[str]) -> list[dict]:
    ...
    Selection and wording come from `ml.inference.reasons_for_row`, the single
    definition of what a reason is, so a transaction's stored evidence can never
    be picked differently from the evidence the API serves live for it.
    """
    reasons = reasons_for_row(contribs[position], features.iloc[position], columns)
    return [
        {
            "feature": r.feature,
            # NaN is not valid JSON; a missing feature stores as null.
