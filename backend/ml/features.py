...
COLUMNS_PATH = PACKAGE_ROOT / "ml" / "feature_columns.json"
    ...
    for path in (tx_path, id_path):
        if not path.exists():
            raise FileNotFoundError(
                f"{path} not found. Set DATA_DIR in .env to the IEEE-CIS folder."
            )
...
def read_column_decisions() -> dict:
    if not COLUMNS_PATH.exists():
        raise FileNotFoundError(
            f"{COLUMNS_PATH} not found. Run `python -m ml.audit` first."
        )
