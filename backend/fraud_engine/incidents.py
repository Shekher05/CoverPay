...
RATE_RATIO_CAP = 1e6  # finite ceiling so the ratio is always JSON-safe
...
class _MerchantWindow:
    ...
    def rate_ratio(self) -> float:
        ...
        if baseline_rate == 0:
            # Unreachable given the append order (the baseline always holds at
            # least the window), but a finite cap keeps this JSON-safe and out
            # of every max()/round() downstream. Well above any severity gate.
            return RATE_RATIO_CAP if window_rate else 0.0
        return min(window_rate / baseline_rate, RATE_RATIO_CAP)
